...
export const list = createRoute({
  ...
  responses: {
    [HttpStatusCodes.OK]: jsonContent(
      ...
      "The paginated list of tasks with filters applied",
    ),
  },
});
...
export const getOne = createRoute({
  ...
  responses: {
    [HttpStatusCodes.OK]: jsonContent(
      taskWithRelationsSchema,
      "The requested Task with relations",
    ),
...
export const bulkUpdate = createRoute({
  ...
  responses: {
    ...
    [HttpStatusCodes.UNPROCESSABLE_ENTITY]: jsonContent(
      createErrorSchema(bulkUpdateSchema),
      "Invalid request",
    ),
  },
});
...
export const bulkDelete = createRoute({
  ...
  responses: {
    ...
    [HttpStatusCodes.UNPROCESSABLE_ENTITY]: jsonContent(
      createErrorSchema(bulkIdsSchema),
      "Invalid request",
    ),
  },
});
...
export const addTags = createRoute({
  ...
  request: {
    params: IdParamsSchema,
    body: jsonContentRequired(
      z.object({ tagIds: z.array(z.number()).min(1) }),
      "Tag IDs to add",
    ),
  },
  responses: {
...
export type BulkDeleteRoute = typeof bulkDelete;
export type AddTagsRoute = typeof addTags;
export type RemoveTagsRoute = typeof removeTags;
