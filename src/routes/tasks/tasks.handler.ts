...
import { categories, subtasks, tags, tasks, taskTags } from "@/db/schema";
...
async function getTaskWithRelations(db: ReturnType<typeof createDb>["db"], taskId: number) {
...
export const list: AppRouteHandler<ListRoute> = async (c) => {
  ...
  let taskIdsWithTag: number[] | undefined;
  if (tagId) {
...
export const create: AppRouteHandler<CreateRoute> = async (c) => {
  ...
  if (tagIds && tagIds.length > 0) {
    await db.insert(taskTags).values(
      tagIds.map(tagId => ({ taskId: inserted.id, tagId })),
    );
  }
...
export const patch: AppRouteHandler<PatchRoute> = async (c) => {
  ...
  if (updates.done !== undefined) {
    (updates as any).completedAt = updates.done ? new Date() : null;
  }
...
export const bulkDelete: AppRouteHandler<BulkDeleteRoute> = async (c) => {
  const { db } = createDb(c.env);
...
export const addTags: AppRouteHandler<AddTagsRoute> = async (c) => {
  const { db } = createDb(c.env);
  const { id } = c.req.valid("param");
  ...
  if (newTagIds.length > 0) {
    await db.insert(taskTags).values(
      newTagIds.map(tagId => ({ taskId: id, tagId })),
    );
  }
...
export const removeTags: AppRouteHandler<RemoveTagsRoute> = async (c) => {
  const { db } = createDb(c.env);
  const { id } = c.req.valid("param");
  const { tagIds } = c.req.valid("json");
