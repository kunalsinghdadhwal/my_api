...
import { categories, subtasks, tasks } from "@/db/schema";

import type { DashboardRoute, ProductivityRoute } from "./stats.routes";

export const dashboard: AppRouteHandler<DashboardRoute> = async (c) => {
...
export const productivity: AppRouteHandler<ProductivityRoute> = async (c) => {
  const { db } = createDb(c.env);
  ...
  const dailyStats = await db.select({
    date: sql<string>`date(${tasks.createdAt} / 1000, 'unixepoch')`.as("date"),
    created: count(),
  })
    .from(tasks)
  ...
  const dailyCompleted = await db.select({
    date: sql<string>`date(${tasks.completedAt} / 1000, 'unixepoch')`.as("date"),
    completed: count(),
  })
    .from(tasks)
    .where(
